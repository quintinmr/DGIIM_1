...
//
/* Programa que calcula la fila de una tabla rectangular que más 
   ...
   vienen dados mediante una secuencia de enteros. La determinación de cuál
   ...
   al él y por tanto será la que se muestre en pantalla. Para ello, en la clase 
   TablaRectangularEnteros en la que se emplea como dato miembro privado una 
   secuencia de secuencias de enteros, con la restricción de que todas las 
   secuencias tienen el mismo tamaño, se ha implementado un método que devuelve
   un objeto de la clse SecuenciEnteros (la fila más parecida), y al que le 
   entran como parámetros dos objetos de la clase SecuenciaEnteros: el vector 
   y las filas a analizar. 
...
public:
	                  ...
	                  SecuenciaEnteros filas_a_analizar)
	{
		...
		for (int k = 0; k < cols_utilizadas; k++){
		   ...
		   pow((vector_a_comparar.Elemento(k),
			    vector_privado[filas_a_analizar.Elemento(0)].Elemento(k)),2);
			                       
			
		fila_parecida.Aniade(vector_privado[filas_a_analizar.Elemento(0)].Elemento(k));
		...
		// analizar tenga  pase a tener más de una fila, comenzamos analizando 
		...
		// con las filas que indicadas en la secuencia de filas a analizar.
		...
		for (int f = 1; f < filas_a_analizar.TotalUtilizados(); f++){
			...
			for (int c = 0; c < cols_utilizadas; c++){
				...
				distancia_entre_elementos = (distancia_entre_elementos + 
				    ...
				    vector_privado[filas_a_analizar.Elemento(f)].Elemento(c)), 
					2));
	...
	/***********************************************************************/
	// Metodo que devuelve una cadena de caracteres que corresponde a la tabla
	// rectangular de enteros.
...
int main ()
{
	...
	for (int i = 0; i < filitas; i++){ 
		...
		lee.SetTitulo("Fila: ");
		filas_analisis.Aniade(lee.LeeEnteroEnRango(1,tabla.FilasUtilizadas()));
